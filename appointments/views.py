    ...
    if request.method == 'POST':
        try:
            ...
            if not one_time:
                ...
                # Initialize the first date for recurring appointments
                ...
                while current_date <= end_date:
                    ...
                    # Increment the date based on the repeat_unit
                    if repeat_unit == 'Week':
                        current_date += timedelta(weeks=repeat_every)
                    elif repeat_unit == 'Month':
                        current_date += timedelta(days=30 * repeat_every)  # Approximation for months
...
def send_contact_site_email(request):
    if request.method == 'POST':
        ...
        if email_error or captcha_error:
